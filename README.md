...
The purpose of this plugin is to download any mods that a level requires that isn't currently installed, and let mappers require their own proprietary plugins!

For example, imagine someone tries to play the mod chart "ANALYS" but they don't have Noodle Extensions. When they click on the level, a notification will appear telling the user they need Noodle Extensions to play it. Then there's an option for you to install the mod and reboot the game without having to switch to a PC! All approved plugins on BeatMods will automatically be capable of downloading within the game, if the level requires it.

But what if you create mod charts and Noodle Extensions just isn't doing it for you? This plugin also allows for you to require your own custom plugins! The download link takes the form of a GitHub link as shown below.
```dat
"_customData" : {
    "_requirements" : [
        "MyCoolModName",
